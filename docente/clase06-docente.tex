...
\item Formalizar el concepto central de Sistemas de almacenamiento, distinguiendo seguridad, progreso y supuestos.
...
\section{Hito del Trabajo Final: Etapa 1}
Al inicio de la clase corresponde la entrega de la Etapa 1 según la especificación provista: problema y alcance, arquitectura, comunicación, tiempo/orden, coordinación básica y análisis preliminar de consenso. La corrección debe respetar estrictamente el contenido habilitante de clases 1--3 y no penalizar por no anticipar Raft o CAP.

\section{Arquitectura didáctica v9 para 180 minutos}
\subsection*{Bloque 1 - desafío inicial (20 min)}
Presentar una situación donde aparezca crash entre WAL y datos, corrupción parcial, compaction atrasada y versiones retenidas. Pedir predicciones individuales antes de introducir vocabulario. El objetivo es hacer visibles los supuestos intuitivos y recuperar el prerequisito de la clase anterior.

\subsection*{Bloque 2 - construcción conceptual (35 min)}
Formalizar WAL, checkpoints, recovery, MVCC, B-trees, LSM trees, Bloom filters y amplificaciones. Separar seguridad, progreso y experiencia del usuario. Explicar no solo \emph{qué} hace cada mecanismo, sino \emph{por qué} existe y qué supuesto permite que funcione.
...
Los grupos aplican P-F-G-S-T a su Trabajo Final. Deben producir un ADR breve con alternativa elegida, alternativa descartada y evidencia pendiente. La evidencia de esta clase es log de recovery, benchmark de lectura/escritura y medición de amplificaciones.
...
\item Explicar el límite, porque crash entre WAL y datos, corrupción parcial, compaction atrasada y versiones retenidas puede invalidar supuestos o reducir progreso.
\item Verificar comprensión mediante log de recovery, benchmark de lectura/escritura y medición de amplificaciones, no mediante una definición aislada.
\end{itemize}
...
Lectura previa recomendada: Kleppmann, capítulo 3; documentación de PostgreSQL sobre WAL/MVCC; lecturas sobre LSM. Preparar un contraejemplo adicional y una variante del caso en la que cambie el costo del error; esto evita convertir la clase en una lectura de diapositivas.
