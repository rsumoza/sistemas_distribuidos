...
\title{Clase 07 --- Guía docente magistral: historia durable, progreso recuperable y efectos correctos}
\author{\UAProfessor}
\date{\UADate}
\begin{document}
\maketitle
...
8--28 & Contexto, puente y vocabulario & Del WAL local al log compartido; CampusShop, identidades y términos previos. \\
28--43 & Diagnóstico e intuiciones & Predicción individual; película, carriles, señaladores y reloj del hecho. \\
43--72 & Log y publicación & Topic, particiones, keys, hot partitions, réplica, ACK e idempotencia. \\
72--82 & Actividad breve & Distribuir eventos entre particiones, justificar key y detectar skew. \\
80--90 & Pausa & Recuperación y preguntas anónimas. \\
92--124 & Consumo y progreso & Grupo, position, committed offset, timelines, rebalance y lag. \\
124--151 & Efectos correctos & Inbox, frontera exactly-once, dual write, outbox y captura de cambios. \\
151--171 & Streams con estado y tiempo & Changelog, ventanas, event time, watermark y eventos tardíos. \\
171--178 & Diseño grupal & Asteria/CampusShop: key, semántica, SLO y prueba de falla. \\
...
\item Preparar 20 tarjetas: seis eventos, seis offsets, dos CRASH, dos consumidores, tres particiones y una tarjeta de ACK.
...
\subsection{Vocabulario mínimo antes de pedir decisiones}

Antes del diagnóstico, dedique quince minutos a construir un lenguaje común. No dé por conocidos términos operativos solo porque aparecen en herramientas reales.

\begin{description}
\item[Broker.] Servidor intermediario que recibe y almacena particiones del log.
\item[Topic.] Categoría lógica dividida en particiones.
\item[Partición.] Carril append-only que ofrece orden local y limita paralelismo.
\item[Key.] Clave que dirige eventos relacionados al mismo carril.
\item[Offset.] Posición dentro de una partición; no es identidad de negocio.
\item[Committed offset.] Progreso durable desde el que reinicia el grupo.
\item[ACK.] Acuse de recibo del broker; su significado depende de qué réplicas debieron aceptar el registro.
\item[ISR.] Réplicas suficientemente al día para la política de confirmación configurada.
\item[SLO.] Objetivo cuantitativo de nivel de servicio, por ejemplo ``99,9\% de pedidos reflejados en inventario en menos de 5 s''.
\end{description}

Use en el pizarrón las tres identidades del caso: \texttt{operation\_id=9F2A}, \texttt{event\_id=evt-9F2A-1} y \texttt{P2, offset 418}. Si el estudiante mezcla estas identidades, no avance todavía hacia exactly-once.
...
\item servicio de pagos externo: queda fuera salvo protocolo/idempotencia propios.
\end{itemize}
...
Dibuje la base de datos y el broker. Pruebe los dos órdenes y coloque un crash entre escrituras. Espere a que el curso concluya que cambiar el orden no resuelve atomicidad.
...
Use un evento ocurrido a las 10:00 que llega a las 10:07. Pida ubicarlo primero por processing time y luego por event time. Señale que llega 2 min 10 s después del cierre 10:05 y pregunte cuánto esperarían para un dashboard y para fraude. La comparación hace emerger el trade-off latencia/completitud.
