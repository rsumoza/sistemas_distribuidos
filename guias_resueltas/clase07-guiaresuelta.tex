...
\title{Clase 07 --- Guía resuelta: del hecho durable al efecto recuperable}
\author{\UAProfessor}
\date{\UADate}
\begin{document}
\maketitle
...
\begin{uaexercise}
\textbf{1. Base mínima: registro, append y progreso.} Dada la secuencia 416--420, un consumidor alcanzó position=421 pero persistió committed offset=418. Explique memoria, estado durable y reinicio.
\end{uaexercise}
\begin{uaanswer}
Los registros 416--420 forman la historia retenida. La instancia viva ya avanzó hasta después de 420, por eso su \emph{position} es 421; esa marca puede existir solo en memoria. El progreso durable del grupo sigue en 418. Si la instancia cae, la memoria desaparece y el nuevo consumidor retoma desde 418, por lo que 418, 419 y 420 pueden reaparecer según la convención del offset comprometido.

\paragraph{Razón.} El sistema solo puede basar la recuperación en estado persistido. La diferencia entre position y committed offset es trabajo leído pero todavía no incorporado al contrato durable del grupo.

\paragraph{Límite.} Que un registro reaparezca no dice si su efecto anterior ocurrió. Para responderlo hace falta el timeline del efecto y, si corresponde, una identidad durable.

\paragraph{Evidencia.} Inyectar un crash después de leer 420, reiniciar el grupo y observar el primer offset entregado y los event IDs repetidos.
\end{uaanswer}
\begin{uaexercise}
...
\end{uaexercise}
\begin{uaanswer}
Con event time, pertenece a la ventana de 10:00--10:05 aunque llegue a las 10:07; puede corregir un resultado ya emitido. Con processing time, se cuenta en 10:07 y distorsiona el momento real. La elección depende de semántica y tolerancia a correcciones.
