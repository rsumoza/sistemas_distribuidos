...
\paragraph{Profundización.} Las reglas pueden incluir módulos declarados, dependencias acíclicas, APIs internas, pruebas de arquitectura, ownership y prohibición de acceso directo a internals.\paragraph{Evidencia.} Grafo de dependencias, ciclos, cambios que cruzan módulos y tiempo de build/test por componente.
\end{uaanswer}
\begin{uaexercise}
...
\paragraph{Profundización.} La prima incluye infraestructura, red, seguridad entre servicios, observabilidad, on-call, contratos, consistencia y costos de coordinación.\paragraph{Criterio.} Separar solo con hipótesis medible: menos lead time, escala independiente o menor blast radius.
\end{uaanswer}
\begin{uaexercise}
...
\paragraph{Profundización.} Recomendaciones suele tener carga/modelo distinto y tolera eventualidad; pagos puede justificar ownership por compliance. Catálogo puede permanecer junto si cambia con el núcleo.\paragraph{Evidencia.} Cadencia de cambio, QPS, incidentes, equipo dueño e invariantes.
\end{uaanswer}
\begin{uaexercise}
...
\paragraph{Profundización.} El anti-patrón paga todos los caminos de red sin obtener autonomía: despliegue, esquema y pruebas siguen coordinados.\paragraph{Métrica.} Porcentaje de releases coordinados y dependencias sincrónicas por request.
\end{uaanswer}
\begin{uaexercise}
...
\paragraph{Profundización.} Las traducciones actúan como anti-corruption layer. Un ID estable puede mapearse, pero los atributos y reglas no deben forzarse a un modelo universal.\paragraph{Trade-off.} Duplicación semántica controlada es preferible a acoplamiento global oculto.
\end{uaanswer}
\begin{uaexercise}
...
\paragraph{Profundización.} Una vista materializada por eventos compra consultas rápidas a costa de frescura; composición RPC compra frescura a costa de disponibilidad/latencia.\paragraph{Criterio.} Elegir por SLO y tolerancia a desactualización, no por conveniencia de SQL.
\end{uaanswer}
\begin{uaexercise}
...
\paragraph{Profundización.} La organización es parte de la arquitectura. Si la frontera cruza continuamente coordinación humana, probablemente el bounded context o el modelo de ownership está mal ubicado.\paragraph{Evidencia.} Revisiones cruzadas, tickets bloqueados y lead time por dependencia de equipo.
\end{uaanswer}
\begin{uaexercise}
...
\paragraph{Profundización.} Pago necesita respuesta inmediata para checkout; notificación y ranking pueden desacoplarse. Perfil puede ser query sincrónica con caché.\paragraph{Regla.} Elegir por necesidad de respuesta, tolerancia al atraso y semántica de error.
\end{uaanswer}
\begin{uaexercise}
...
\paragraph{Profundización.} $0{,}999^4$ es un límite optimista: las fallas no son independientes y la red agrega otra dependencia. La latencia p99 no se calcula sumando medias.\paragraph{Mitigación.} Reducir profundidad, usar fallbacks, paralelizar cuando sea seguro y definir presupuestos de timeout.
\end{uaanswer}
\begin{uaexercise}
...
\paragraph{Profundización.} Un comando tiene destinatario/owner y puede ser rechazado; un evento ya ocurrió y puede tener muchos consumidores.\paragraph{Riesgo.} Comandos disfrazados de eventos ocultan dependencia obligatoria y manejo de errores.
\end{uaanswer}
\begin{uaexercise}
...
\paragraph{Profundización.} La ventana persiste incluso si ambas operaciones son idempotentes: puede faltar una de ellas. Además, un retry tras ACK perdido puede repetir publicación.\paragraph{Prueba.} Crash injection en cada corte y reconciliación posterior.
\end{uaanswer}
\begin{uaexercise}
...
\paragraph{Profundización.} La secuencia por aggregate evita reorder de eventos del mismo modelo. El relay requiere locking/claiming para trabajo concurrente y política de retry.\paragraph{Métricas.} Edad p99, filas pendientes, publicaciones duplicadas y poison events.
\end{uaanswer}
\begin{uaexercise}
...
\paragraph{Profundización.} Los consumidores deben aceptar campos desconocidos y el productor no debe cambiar significado silenciosamente. Puede usarse dual publishing o upcasting durante transición.\paragraph{Evidencia.} Contract tests contra versiones soportadas y replay de eventos históricos.
\end{uaanswer}
\begin{uaexercise}
...
\paragraph{Profundización.} Cada paso se modela como estado durable y comando idempotente. Los deadlines distinguen retry de compensación.\paragraph{Criterio.} No compensar un paso secundario como email deshaciendo todo el negocio.
\end{uaanswer}
\begin{uaexercise}
...
\paragraph{Profundización.} La compensación conserva historia y puede tener costo económico/temporal. Debe admitir retry y, cuando falla, escalar a intervención manual.\paragraph{Métrica.} Sagas compensating, antigüedad y monto expuesto.
\end{uaanswer}
\begin{uaexercise}
...
\paragraph{Profundización.} Correlation ID y una vista de seguimiento hacen visible el flujo. Reglas de dependencia evitan que eventos creen ciclos.\paragraph{Límite.} Al aumentar participantes, la lógica emergente se vuelve difícil de modificar y auditar.
\end{uaanswer}
\begin{uaexercise}
...
\paragraph{Profundización.} El orquestador es una máquina de estados replicada; no debe guardar progreso solo en memoria. Versionar workflows evita romper sagas en curso.\paragraph{Trade-off.} Mejora visibilidad a costa de un componente más crítico y conocimiento central del proceso.
\end{uaanswer}
\begin{uaexercise}
...
\paragraph{Profundización.} El primer servicio debe tener frontera observable y rollback. Durante coexistencia, un anti-corruption layer y una fuente de verdad clara evitan dual ownership.\paragraph{Métrica.} Comparar lead time, p99, error rate, costo y frecuencia de rollback antes de continuar.
\end{uaanswer}
\begin{uaexercise}
...
\paragraph{Profundización.} La checklist debe incluir también capacidad humana: equipo dueño, rotación on-call, runbooks y presupuesto de toil.\paragraph{Criterio.} Si cada incidente requiere al arquitecto original, la arquitectura no es operable.
\end{uaanswer}
\begin{uaexercise}
...
\paragraph{Profundización.} La propuesta debe explicar por qué pagos/video compran autonomía real y por qué aprendizaje queda modular. Cada frontera incluye datos, contrato, SLO y falla.\paragraph{Prueba.} Medir despliegue independiente, blast radius, dual-write consistency y experiencia de inscripción bajo fallas.
\end{uaanswer}
