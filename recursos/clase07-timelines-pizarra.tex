...
\section{Registro, append y progreso durable}

Use esta hoja antes de introducir particiones y offsets. Coloque registros en la secuencia, mueva el lector y marque por separado su posición actual y el progreso persistido.

\begin{center}
\begin{tikzpicture}[x=1.15cm,y=1cm,>=Latex]
\node[anchor=east,font=\bfseries] at (0,2){Log};
\foreach \x in {1,...,11}{
  \node[draw=UAIngNavy,rounded corners,minimum width=0.78cm,minimum height=0.52cm] at (\x,2){};
}
\node[anchor=east,font=\bfseries] at (0,0.8){Lector};
\draw[UAIngRule,line width=1pt] (0.6,0.8)--(12,0.8);
\node[anchor=east,font=\bfseries] at (0,-0.4){Progreso durable};
\draw[UAIngRule,line width=1pt] (0.6,-0.4)--(12,-0.4);
\end{tikzpicture}
\end{center}

\begin{enumerate}
\item Complete la secuencia con 416--420. \vspace{0.55cm}
\item Coloque la posición actual después de 420. \vspace{0.55cm}
\item Coloque el progreso durable en 418. \vspace{0.55cm}
\item ¿Qué desaparece con el crash? \hrulefill
\item ¿Qué registros reaparecen al reiniciar? \hrulefill
\end{enumerate}
