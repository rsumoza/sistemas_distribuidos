...
\title{Clase 07 --- Ficha de aula: del hecho durable al efecto recuperable}
\author{\UAProfessor}
\date{\UADate}
\begin{document}
\maketitle
...
\section{Base mínima: historia y progreso recuperable}

Complete antes de utilizar vocabulario de producto.

\begin{tabular}{p{0.23\textwidth}p{0.29\textwidth}p{0.34\textwidth}}
\toprule
Objeto & Mi explicación inicial & ¿Qué ocurre después de un crash? \\
\midrule
Memoria de trabajo & \rule{0pt}{0.9cm} & \\
Registro & \rule{0pt}{0.9cm} & \\
Append & \rule{0pt}{0.9cm} & \\
Log persistente & \rule{0pt}{0.9cm} & \\
Progreso durable & \rule{0pt}{0.9cm} & \\
\bottomrule
\end{tabular}
...
[416,417,418,419,420]
\]
El consumidor llegó a \texttt{position=421}, pero persistió \texttt{committed=418}.

\begin{enumerate}
\item ¿Qué marca puede estar solo en memoria? \hrulefill
\item ¿Desde dónde reinicia? \hrulefill
\item ¿Qué registros pueden reaparecer? \hrulefill
\end{enumerate}

\section{Intuiciones que luego formalizaremos}
\begin{tabular}{p{0.18\textwidth}p{0.34\textwidth}p{0.34\textwidth}}
\toprule
Concepto & Mi intuición inicial & Definición corregida al final \\
\midrule
Log & \rule{0pt}{0.9cm} & \\
Partición & \rule{0pt}{0.9cm} & \\
Offset & \rule{0pt}{0.9cm} & \\
Consumer group & \rule{0pt}{0.9cm} & \\
Idempotencia & \rule{0pt}{0.9cm} & \\
Outbox & \rule{0pt}{0.9cm} & \\
\bottomrule
\end{tabular}

\newpage
\section{Identidades del caso CampusShop}
...
\newpage
\section{Publicación, réplica y ACK --- extensión}
...
\newpage
\section{Timeline de offset y crash}
...
\newpage
\section{Frontera de exactly-once --- extensión}
...
\newpage
\section{Event time y eventos tardíos --- extensión}
Eventos:
\begin{center}
\begin{tabular}{cccc}
\toprule
...
\newpage
\section{Diagnóstico de lag y operación}
