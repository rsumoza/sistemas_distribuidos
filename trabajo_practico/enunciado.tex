...
\begin{document}
    ...
    \subsection{Modalidad sugerida}\label{subsec:modalidad-sugerida}
    ...
    \begin{table}[H]
        ...
        \begin{tabular}{p{0.28\textwidth}p{0.62\textwidth}}
            ...
            \midrule
            Equipos & 3 o 4 estudiantes. \\
            Entregables & Un documento por entrega con los pares PFGST acumulados, más la defensa oral al cierre. \\
            Defensa & 10 minutos de presentación y 5 minutos de preguntas del comité.
            Cualquier integrante puede ser consultado sobre cualquier decisión del trabajo. \\
            \bottomrule
        \end{tabular}
        \caption{Modalidad de trabajo y evaluación.}\label{tab:modalidad}
    \end{table}

    La dedicación estimada se reparte de forma despareja entre las tres entregas: la Entrega 2 concentra el grueso del trabajo, porque es donde aparecen las soluciones técnicas y las decisiones de CAP. Conviene planificar en consecuencia y no dejarla para la última semana.

    \begin{table}[H]
        \centering
        \begin{tabular}{p{0.28\textwidth}p{0.20\textwidth}p{0.42\textwidth}}
            \toprule
            \textbf{Entrega} & \textbf{Dedicación} & \textbf{Dónde se va el tiempo} \\
            \midrule
            Entrega 1 & 10 a 14 horas & Menos en escribir que en reformular: es habitual descartar la primera tanda de garantías. \\
            Entrega 2 & 25 a 30 horas & Diseño de las soluciones, decisión de CAP por subsistema y el par con dos soluciones alternativas. \\
            Entrega 3 & 15 a 20 horas & Trade-offs, análisis adversarial y preparación de la defensa. \\
            \midrule
            \textbf{Total} & \textbf{50 a 65 horas} & Horas de trabajo de equipo, no por persona. \\
            \bottomrule
        \end{tabular}
        \caption{Dedicación estimada por entrega para un equipo de 3 integrantes.}\label{tab:dedicacion}
    \end{table}

    \subsection{Fuera de alcance}\label{subsec:fuera-de-alcance}

    El foco del trabajo está puesto en la gestión distribuida del estado del juego y en las operaciones que lo sostienen.
    Quedan fuera de alcance:

    \begin{itemize}
        \item El diseño de la interfaz gráfica del jugador: pantallas, navegación, animaciones y efectos visuales.
        Atención: esto excluye el diseño visual, no el comportamiento del cliente.
        Cómo reacciona la aplicación móvil ante una desconexión o un reintento sí forma parte del trabajo, porque es donde se originan las fallas de cliente (ver sección~\ref{sec:modelo-fallas}).
        \item La implementación: no se pide escribir código, ni elegir productos o tecnologías concretas.
        Las soluciones se describen a nivel de mecanismo, no de producto.
        \item El diseño de mecánicas de juego nuevas: no hace falta inventar zonas, enemigos, habilidades ni economía adicional.
        El contexto de juego se toma como dado, y el trabajo consiste en encontrar los problemas distribuidos que ese contexto genera.
    \end{itemize}

    \section{Contexto de juego}\label{sec:contexto-juego}
    ...
    \subsection{Zonas e instancias}\label{subsec:zonas-instancias}
    ...
    \subsection{Combate}\label{subsec:combate}
    ...
    Además, el jugador que asesta el golpe final al jefe recibe un premio adicional.
    ...
    \subsection{Objetos y mercado}\label{subsec:objetos-mercado}
    ...
    \subsection{Casos de uso de referencia}\label{subsec:casos-de-uso}
    ...
    \section{Contexto de infraestructura}\label{sec:contexto-infraestructura}
    ...
    La comunicación entre los clientes y los datacenters, y también la comunicación entre datacenters, debe utilizar el protocolo gRPC para hacer más eficiente el intercambio de información.

    \subsection{Datacenter}\label{subsec:datacenter}
    ...
    \subsection{Casos especiales multi-datacenter: evento global y directorio de zonas}\label{subsec:casos-multi-datacenter}
    ...
    \subsection{Caída vs. particionamiento}\label{subsec:caida-particionamiento}
    ...
    Puede haber fallas parciales de infraestructura (caída del mapa de zonas, combates o pagos).

    \section{Contexto de negocio}\label{sec:contexto-negocio}
    ...
    \section{Modelo de fallas}\label{sec:modelo-fallas}
    ...
    \begin{itemize}
        ...
        \item Fallas de infraestructura: caída completa de un datacenter, o particionamiento temporal entre datacenters que siguen operativos, pero sin verse entre sí (ver sección~\ref{sec:contexto-infraestructura}).
    ...
    \section{Entregas}\label{sec:entregas}

    \subsection{Entrega 1 - Problemas, Garantías y Fallas de cliente}\label{subsec:entrega-1}
    ...
    Los tres casos de uso de la sección~\ref{subsec:casos-de-uso} son una guía y un punto de partida sugerido, pero no son de uso obligatorio ni excluyente: se puede identificar cualquier problema dentro del contexto de juego, infraestructura y negocio ya definido (secciones~\ref{sec:contexto-juego},~\ref{sec:contexto-infraestructura} y~\ref{sec:contexto-negocio}).
    Antes de escribir las garantías, releer la sección~\ref{sec:garantias}: la forma en que se enuncia una garantía es lo que va a permitir (o impedir) que quede fija durante las tres entregas.
    ...
    \subsection{Entrega 2 - Incorporamos fallas de infraestructura y ofrecemos soluciones}\label{subsec:entrega-2}
    ...
    \subsection{Entrega 3 - Incorporamos fallas de seguridad y damos los trade-off}\label{subsec:entrega-3}
    ...
    Si alguna solución sostiene su garantía solo de forma parcial (ver sección~\ref{sec:garantias}), hay que explicitar en el trade-off cuál es exactamente la brecha: bajo qué condiciones el interés protegido queda expuesto, y por qué se aceptó esa exposición en lugar de otra alternativa.
