...
\item ¿Qué mecanismo de Resiliencia distribuida es pertinente y cuál sería sobreingeniería?
...
\begin{frame}{Retry budget}
Un request con tres capas y dos retries por capa puede multiplicarse hasta $3^3=27$ intentos. Limitar retries localmente no alcanza si el presupuesto no es extremo a extremo.
\end{frame}
\begin{frame}{Backpressure vs load shedding}
Backpressure desacelera al productor; load shedding rechaza trabajo que ya no puede procesarse dentro del deadline. Ambos protegen estabilidad, pero ofrecen semánticas distintas al cliente.
\end{frame}
\begin{frame}{Checkpoint de resiliencia}
Para el camino crítico, declare timeout total, presupuesto de retries, límite de concurrencia, fallback, señal de breaker y degradación aceptable.
\end{frame}
