...
\item ¿Qué mecanismo de Chaos Engineering y validación bajo falla es pertinente y cuál sería sobreingeniería?
...
\begin{frame}{Experimento, no espectáculo}
Un experimento válido conecta hipótesis, perturbación, steady state y criterio de aborto. ``Apagar cosas y mirar'' no produce evidencia reproducible.
\end{frame}
\begin{frame}{Escalera de seguridad}
\begin{enumerate}
\item test determinista local;
\item entorno de integración;
\item porcentaje pequeño de tráfico;
\item producción con blast radius y abort automático.
\end{enumerate}
\end{frame}
\begin{frame}{Checkpoint experimental}
Escriba antes de ejecutar: qué espera observar, qué resultado refuta la hipótesis y qué mecanismo de resiliencia está realmente evaluando.
\end{frame}
