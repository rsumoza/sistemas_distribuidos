...
\begin{itemize}
\item Consenso $\Rightarrow$ orden total de operaciones
...
Si el grafo tiene ciclo $\Rightarrow$ no serializable
\end{alertblock}
\end{frame}
...
\item $T_1$ vuelve a leer $\Rightarrow$ aparece “fantasma”
\end{itemize}
\end{frame}
...
Snapshot Isolation permite esto $\Rightarrow$ no es serializable
\end{alertblock}
\end{frame}
...
\textbf{Fase 2: Commit}
\begin{itemize}
\item si todos dicen sí $\Rightarrow$ commit
\item si alguno dice no $\Rightarrow$ abort
\end{itemize}
\end{frame}
